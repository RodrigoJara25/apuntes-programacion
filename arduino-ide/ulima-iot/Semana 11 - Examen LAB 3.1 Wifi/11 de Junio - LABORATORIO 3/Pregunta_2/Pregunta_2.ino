...
int analog1 = 36;  //potenciometro
int analog2 = 39;  //ldr
int analog3 = 34; //lm35
int digital = 35; //digital
/**************************Setup*************************************/
void setup() {
  Serial.begin(115200);
...
void loop() {
  ...
  int val1 = analogRead(analog1);  //potenciometro
  int val2 = analogRead(analog2);   //ldr
  int val3 = analogRead(analog3);   //lm35
  int val4 = digitalRead(digital);   //boton
  if ((millis() - lastTime) > timerDelay) {
    ...
    if(WiFi.status()== WL_CONNECTED){
      ...
      String httpRequestData = "api_key=" + apiKey + "&field1=" + String(val1) + "&field2=" + String(val2) + "&field3=" + String(val3) + "&field4=" + String(val4);      
