...
#define IO_USERNAME  "RodrigoJ"   //cambiar esto
#define IO_KEY       "aio_JHIE507avSxK4R42poRZP4Dz11Lm"   //cambiar esto
...
Adafruit_MQTT_Subscribe encendido = Adafruit_MQTT_Subscribe(&mqtt, IO_USERNAME "/feeds/onoff20241");  //poner nombre del FEED del LAYOUT (CAMBIAR ESTO)
...
void digitalCallback(char *data, uint16_t len) {
  ...
  Serial.println(data);
     ...
     String message = String(data);
      message.trim();
      if (message == "ON") {digitalWrite(led, HIGH);} //CUIDADO CON ESTO
      if (message == "OFF") {digitalWrite(led, LOW);}  //CUIDADO CON ESTO
}
